...
요구사항은 평시 `50 TPS`, 프로모션 오픈 직후 `1~5분` 동안 `500~1000 TPS` 급증을 가정합니다. 또한 인프라 증설이 제한적이고, `2개 이상`의 WAS replica를 전제로 합니다. 최종 정합성 상태는 WAS local state가 아니라 MySQL/Redis에 둡니다.

이 조건에서 모든 요청을 깊은 DB transaction까지 들여보내면 성공 예약은 10건뿐인데 실패 요청이 DB를 압박합니다. 그래서 정상 경로는 Redis가 빠르게 후보를 거르고, MySQL은 `booking_admission` 예약 접수 원장과 최종 정합성 원장으로 사용합니다. 상품 가격/오픈 시각 같은 metadata read는 짧은 TTL cache로 보호하며, 핵심 목표는 inventory write path를 피크 전체 요청에서 분리하는 것입니다.

문서에서 `admission ledger`는 `booking_admission`에 남는 예약 접수 원장을 뜻합니다. `db_admission_seq`는 이 원장에 저장된 DB 확정 순서입니다.

Redis 장애 fallback은 아래 원칙을 따릅니다.

- Redis는 최종 재고 원장이 아닙니다. 정상 경로에서는 Redis HA가 후보를 빠르게 거르고, MySQL 예약 접수 원장이 DB 확정 순서를 남깁니다.
- Redis failover 중 request thread는 DB 접수 처리로 직접 우회하지 않습니다.
- Kafka/MQ 같은 durable queue는 전역 순서와 내구성을 더 잘 보장하지만, 현재 범위에서는 HA queue 운영 비용과 consumer 복잡도를 피하기 위해 작은 WAS-local bounded queue를 선택합니다.
- Local queue는 일부 요청만 `202 LOCAL_QUEUE_ACCEPTED`로 받고, background worker가 제한된 속도로만 MySQL 예약 접수 원장을 갱신합니다.
- Queue capacity 또는 장애 episode 수용 예산을 넘은 요청은 `429 LOCAL_QUEUE_FULL`로 버립니다.
- Worker가 DB busy/timeout을 만나면 즉시 완료 처리하지 않고 retry/backoff 후 재큐잉하며, retry budget이나 max age를 넘을 때만 terminal unavailable로 고정합니다.
- Redis half-open probe가 성공해도 local queue가 비거나 복구 시점부터 drain-grace가 지날 때까지 새 요청은 local queue에 유지합니다.

`booking_attempt_id`의 저장 위치는 의도적으로 분리합니다.

- Checkout 응답 직후의 `booking_attempt_id`는 서버가 발급한 signed attempt token이며, 아직 DB row가 아닐 수 있습니다.
- Redis 후보 상태에는 `booking_attempt_id`를 저장하지 않습니다. Redis에는 사용자별 `userId -> redisSeq`와 ZSET 순번만 저장합니다.
- DB에는 `POST /bookings` 처리 중 `idempotency_record`, `booking_admission`, `reservation`, `payment_attempt`, `point_hold`에 단계별로 저장됩니다.
- Redis 장애 중 `LOCAL_QUEUE_ACCEPTED` 직후부터 worker가 DB 예약 접수 원장에 기록하기 전까지의 짧은 구간은 WAS local memory queue에만 존재합니다. 이는 Kafka/MQ를 도입하지 않은 trade-off입니다.

PG timeout/응답 유실은 즉시 성공이나 실패로 단정하지 않습니다. 먼저 `PAYMENT_UNKNOWN`으로 저장하고 recovery worker가 PG status/cancel을 자동 확인합니다. 재고 점유는 `30s` deadline까지만 허용하며, `5분` reconciliation window 이후에도 결제/취소 상태가 불명확할 때만 payment-only `MANUAL_REVIEW_REQUIRED`로 넘깁니다.
...
flowchart LR
    ...
    AppA["Booking Service A<br/>normal path stateless<br/>degraded local queue"]
    AppB["Booking Service B<br/>normal path stateless<br/>degraded local queue"]
    RedisPrimary["Redis primary<br/>접수 gate"]
...
| Redis HA | 정상 경로 접수 gate, 중복 사용자 빠른 차단, 후보군 제한 | 최종 재고 원장이 아님 |
| WAS-local queue | Redis 장애 중 bounded in-memory 대기열과 throttled worker 제공 | durable/global FIFO 원장이 아님 |
| MySQL | 예약 접수 원장, reservation, idempotency, payment 상태 저장 | 최종 재고 정합성 원장 |
| Booking Service | 예약/결제 흐름 조정, bulkhead, recovery scheduler | 최종 상태는 DB/Redis에 저장. Redis 장애 중 transient local queue는 예외 |
...
| `POST` | `/api/v1/bookings` | 멱등성 확인, 후보 판단, 재고 점유, PG confirm, 예약 확정 | 구현 |
| `GET` | `/api/v1/bookings/status/{bookingAttemptId}` | local queue, payment unknown, reservation 상태 polling | 구현 |
...
현재 구현은 local/test/load-test 장애 주입을 위해 request DTO에 `mock_pg_scenario`를 포함합니다. k6와 통합 테스트는 `SUCCESS`, `FAILURE`, `TIMEOUT`, `LATE_SUCCESS`로 실제 PG의 승인/조회/취소/응답 유실 흐름을 흉내냅니다. 이 필드는 운영 API 계약이 아니며, production hardening 단계에서는 public request body에서 제거하거나 profile/internal boundary로 격리해야 합니다.
...
sequenceDiagram
    ...
    alt Redis 정상 접수
        ...
        alt queue accepted
            ...
            Q->>D: throttled worker DB 예약 접수 원장 기록
...
| `booking_admission` | `(sale_event_id, product_id, user_id)` | 같은 사용자는 한 이벤트 상품에서 한 번만 접수 기회 획득 |
| `booking_admission` | `(sale_event_id, product_id, db_admission_seq)` | DB 확정 순서 중복 방지 |
...
- 단위 테스트: 결제 조합, request hash, 접수 gate 판단
