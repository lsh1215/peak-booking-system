...
| DEC-008 | 테스트, load-test, observability 전략 | 부분 수용 | User | 2026-05-31 |
...
- candidate pool 크기, gateway/app rate limit, semaphore, DB connection budget은 DEC-007의 초기 runtime budget으로 산정한다. 최종값은 k6/LGTM 검증 후 조정한다.
...
초기값은 수용하지만, 실제 p95/p99, Hikari pending, DB lock wait, CPU/heap, PG unknown drain 결과에 따라 k6/LGTM 검증 후 조정해야 한다.
...
| 재고 불변식 | `confirmed_count + reserved_count <= total_stock` |
...
| PG unknown | 즉시 success/release로 조용히 확정하지 않음 |
...
- Mock PG가 최종 상태를 제공하는 `PAYMENT_UNKNOWN`은 `60s` 안에 drain되어야 한다.
...
구현 후 concrete metric name, dashboard layout, alert rule, k6 script path를 확정해야 한다. 초기 threshold는 첫 검증 결과에 따라 조정한다.
