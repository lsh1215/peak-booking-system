...
| mixed | `100/s`, `45s` | latency threshold fail | confirmed `10` | k6 confirmed response counter `12`, DB confirmed `10` |
...
| late-success after `330s` | `100/s`, `45s` + wait | k6 통과, 후속 DB 관찰 | confirmed `5`, released `5`, payment `MANUAL_REVIEW_REQUIRED 5` | 재고/예약은 되살리지 않음. 다만 cancel 자동 완료가 아니라 수동 확인으로 닫힘 |
...
5. k6 `booking_confirmed_total`은 confirmed 응답/replay까지 셀 수 있으므로 oversell hard fail 근거로 사용하지 않는다. DB snapshot이 authoritative evidence다.
