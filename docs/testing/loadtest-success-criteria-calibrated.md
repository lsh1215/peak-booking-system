...
> 2026-06-03 Redis HA 설계 반영 후 Redis 장애 시나리오의 기준은 `Redis hard-down 중 DB fallback 판매`가 아니라 `Redis HA failover 중 controlled rejection + half-open recovery`다. 아래 2026-06-01 `redis-down` 기록은 단일 Redis 후보를 검증하던 역사적 결과로 남긴다.
...
4. `redis-down` 단일 Redis 실험은 oversell은 없었지만 DB fallback 판매가 아니라 controlled Redis rejection으로 관측됐다. 이후 설계는 bounded DB fallback을 기본값에서 제거하고 Redis HA + failover pause + half-open recovery로 변경했다. 따라서 현재 Redis 장애 pass/fail은 Redis master failover 중 fast-fail, DB fallback 우회 없음, half-open recovery 성공 여부로 판단한다.
