...
이 문서는 실제 k3s/GCP 부하 테스트 결과를 반영해 보정한 성공 기준이다. 초기 예상 기준은 [부하 테스트 성공 기준 - 초기 예상 기준](./loadtest-success-criteria-initial.md)에 보존한다. 문서 주장과 테스트 증거의 연결 상태는 [부하 테스트 증거 인덱스](./loadtest-evidence-index.md)에 따로 정리한다.

> 2026-06-03 Redis HA 설계 반영 후 Redis 장애 시나리오의 기준은 `Redis hard-down 중 DB fallback 판매`가 아니라 `Redis HA failover 중 controlled rejection + half-open recovery`다. 아래 2026-06-01/2026-06-02 `redis-down` 기록은 단일 Redis/DB fallback 후보를 검증하던 역사적 결과로 남긴다. Redis master failover k6 수치는 최신 코드 기준으로 다시 실행해 증거 인덱스에 연결해야 한다.
...
| redis-down | `0` | confirmed `0`, occupied `0` | Redis scale-to-zero에서는 DB fallback 판매가 아니라 controlled Redis rejection으로 관측 | 역사적 참고. 현재 기준은 Redis HA failover 시나리오로 대체 |
...
| Redis 장애 | oversell과 unlimited DB fallback이 없어야 한다. 현재 채택 기준은 Redis HA failover 중 새 admission을 DB fallback으로 우회하지 않고 controlled rejection + half-open recovery를 수행하는 것이다 |
...
| Redis HA failover controlled rejection | p95 `<= 500ms` | p95 `> 300ms` | half-open probe 억제 후 failover pause는 빠른 Retry-After 응답이어야 한다. Redis hard-down scale-to-zero 실측은 역사적 참고값으로만 유지 |
...
| Redis HA failover / Redis degraded | expected iterations의 `< 1%`. Redis hard-down scale-to-zero 진단은 별도 실험으로 분리 |
...
4. `redis-down` 단일 Redis 실험은 oversell은 없었지만 DB fallback 판매가 아니라 controlled Redis rejection 또는 역사적 DB fallback 후보로 관측됐다. 이후 설계는 bounded DB fallback을 기본값에서 제거하고 Redis HA + failover pause + half-open recovery로 변경했다. 따라서 현재 Redis 장애 pass/fail은 Redis master failover 중 fast-fail, DB fallback 우회 없음, half-open recovery 성공 여부로 판단한다. 이 최신 HA failover 실측 결과는 아직 repository 문서와 직접 연결되지 않았으므로 재실측 후 [증거 인덱스](./loadtest-evidence-index.md)에 추가한다.
