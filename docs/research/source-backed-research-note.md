...
|---|---|---|---|---|
| Redis Lua | Redis Lua script는 여러 Redis operation을 하나의 server-side atomic step으로 묶는 데 유용하다. | 검증됨 | [Redis Lua scripting docs](https://redis.io/docs/latest/develop/programmability/eval-intro/) | 쟁점 2의 Redis admission/coordination 구현 근거다. |
...
| Redis `WAIT` | Redis `WAIT`는 같은 connection의 이전 write가 지정한 replica 수에 ACK될 때까지 block한다. Redis replication 문서는 `WAIT`가 ACK된 copy 수를 보장할 수 있지만 Redis 집합을 강한 일관성 CP system으로 만들지는 않으며, failover 중 acknowledged write가 여전히 유실될 수 있다고 설명한다. | 검증됨 | [Redis WAIT](https://redis.io/docs/latest/commands/wait/), [Redis replication](https://redis.io/docs/latest/operate/oss_and_stack/management/replication/) | Redis HA admission에서 `WAIT 1`은 seq 유실 가능성을 낮추는 완화책이다. MySQL admission ledger 저장 전 Redis seq를 최종 공정성 원장으로 보면 안 된다. |
| Redis replica write safety knobs | Redis replication은 `min-replicas-to-write`, `min-replicas-max-lag`를 통해 replica가 충분하지 않거나 lag가 큰 경우 primary write를 제한할 수 있다고 설명한다. | 검증됨 | [Redis replication](https://redis.io/docs/latest/operate/oss_and_stack/management/replication/) | HA 모드에서 replica ACK 없는 Redis admission write를 신뢰하지 않기 위한 방어 설정이다. write가 거절되면 DB fallback이 아니라 failover pause로 처리한다. |
| Redis Sentinel failover | Redis Sentinel은 master 장애를 감지하고 replica promotion을 수행하는 HA mechanism을 제공한다. Sentinel failover에는 quorum과 majority authorization이 관여한다. | 검증됨 | [Redis Sentinel](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/) | self-managed HA 후보는 primary 1 + replica 2 + Sentinel 3 구조를 기준으로 검증한다. failover window에는 app이 새 admission을 pause하고 half-open probe로 회복을 확인해야 한다. |
...
| Idempotency replay/TTL | Stripe는 idempotency key로 들어온 첫 요청의 status code/body를 저장하고 이후 retry에 같은 결과를 반환하며, 같은 key의 parameter를 비교하고, 최소 24시간 이후 key를 pruning할 수 있다고 설명한다. | 검증됨 | [Stripe idempotent requests](https://docs.stripe.com/api/idempotent_requests) | Stored response replay, request comparison, `24h` retention은 쟁점 3의 수용된 정책 근거다. 이 프로젝트의 key는 client 임의 값이 아니라 서버 발급 `booking_attempt_id`다. |
| Limited inventory checkout expiry | Stripe는 제한 재고 상품에서 사용자가 재고를 오래 잡고 있지 않도록 Checkout Session 만료를 사용하라고 설명하며, session expiration 시 pending sale을 취소하고 재고를 반환하는 흐름을 안내한다. | 검증됨 | [Stripe Checkout: Manage limited inventory](https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=embedded-components) | `PAYMENT_UNKNOWN`은 내부 payment reconciliation 대상으로 추적하더라도, 재고 점유는 짧은 deadline 안에서만 허용하고 이후 다음 후보에게 판매 기회를 넘기는 근거다. |
| Payment idempotency latest status | PayPal은 timeout/5xx처럼 결과가 불명확한 요청을 같은 idempotency id로 재시도할 수 있고, 같은 id로 이전 요청의 최신 상태를 반환한다고 설명한다. | 검증됨 | [PayPal API idempotency](https://developer.paypal.com/reference/guidelines/idempotency/) | `PAYMENT_UNKNOWN` 재시도는 새 PG confirm을 만들지 않고 status/replay 경로로 연결해야 한다. |
...
| Toss Payments idempotency guidance | Toss Payments는 idempotency-key의 replay, in-progress conflict, same-key-different-payload rejection scenario를 설명한다. | 검증됨 | [Toss Payments idempotency blog](https://docs.tosspayments.com/blog/what-is-idempotency) | 쟁점 3의 request hash conflict, in-progress 상태 처리, replay 정책의 외부 참고 근거다. |
...
| Retry safety | AWS는 side-effecting API retry가 idempotency 없이는 안전하지 않다고 경고한다. | 검증됨 | [AWS Builders Library: timeouts, retries, and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) | 쟁점 3/6은 반복 payment request가 duplicate side effect를 만들지 않도록 해야 한다. |
| Retry storm | AWS는 retry를 분산하고 congestion을 줄이기 위해 backoff와 jitter를 권장한다. | 검증됨 | [AWS Builders Library: timeouts, retries, and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/) | Retry cap, backoff, jitter, fast failure는 쟁점 6 overload control 후보이다. |
| Payment reconciliation active query window | AWS는 remote call timeout을 downstream latency percentile과 padding으로 정하고, side-effecting retry는 idempotency와 capped backoff/jitter로 제한해야 한다고 설명한다. PayPal은 timeout/5xx 이후 같은 idempotency id로 최신 상태를 반환하는 방식을 설명한다. | 프로젝트 가정 | [AWS Builders Library: timeouts, retries, and backoff with jitter](https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/), [PayPal API idempotency](https://developer.paypal.com/reference/guidelines/idempotency/) | 이 프로젝트는 Mock PG normal delay `100ms`, 사용자-facing wait `60s`, worker backoff `5s -> 15s -> 45s -> 2m -> 5m`를 고려해 payment_attempt 적극 status/cancel reconciliation window를 `5분` 초기값으로 둔다. 이는 재고 점유 시간이 아니며, 재고 점유 deadline은 별도 `30s`로 둔다. 최종 운영값은 k6/LGTM과 PG mock fault injection으로 검증한다. |
...
| Inventory reservation source of truth | Shopify는 Redis/MySQL split-state failure mode를 겪은 뒤 inventory reservation과 ledger update를 ACID guarantee 아래 두기 위해 reservation을 Redis에서 MySQL로 옮겼다. | 검증됨 | [Shopify Engineering: We replaced Redis with MySQL for inventory reservations](https://shopify.engineering/scaling-inventory-reservations) | 쟁점 1에서 RDB-backed inventory correctness를 평가하는 근거가 된다. 다만 이 자료만으로 프로젝트 구현이 결정되는 것은 아니다. |
...
| Waiting room user feedback | Cloudflare Waiting Room은 FIFO 순서를 설명하고, 대기 중 browser가 주기적으로 새로고침되어 예상 대기 시간을 갱신한다고 설명한다. | 검증됨 | [Cloudflare Waiting Room: About](https://developers.cloudflare.com/waiting-room/about/) | 대기 후보를 둘 경우 backend 내부 상태가 아니라 사용자에게 짧고 예측 가능한 대기 window/status를 제공해야 한다. |
| Distributed fallback risk | AWS는 distributed fallback이 테스트하기 어렵고, 보호 계층을 우회해 dependency로 direct load를 보내면 outage를 악화시킬 수 있다고 경고한다. | 검증됨 | [AWS Builders Library: Avoiding fallback in distributed systems](https://aws.amazon.com/builders-library/avoiding-fallback-in-distributed-systems/) | Redis failure 중 unlimited direct DB fallback은 장애를 증폭할 수 있다. 현재 결정은 Redis HA + failover pause를 기본으로 두고, DB fallback으로 새 admission을 만들지 않는다. |
| K3s ingress baseline | K3s는 기본 packaged ingress controller로 Traefik을 배포하며, 현재 K3s docs는 K3s version에 따라 Traefik v3 지원 여부가 달라질 수 있음을 설명한다. | 검증됨 | [K3s Networking Services](https://docs.k3s.io/networking/networking-services), [K3s Upgrades](https://docs.k3s.io/upgrades) | k3s + Traefik을 쓴다면 별도 commercial gateway product를 가정하지 않고 쟁점 6에서 gateway-level middleware를 검토할 수 있다. |
| Traefik rate limiting | Traefik RateLimit은 token-bucket 기반이며, `average`/`period`는 refill rate를, `burst`는 bucket size를 정의한다. Redis-backed configuration은 distributed rate limiting을 제공할 수 있고, in-memory rate limiting은 proxy instance별로 동작한다. | 검증됨 | [Traefik v3.5 RateLimit middleware](https://doc.traefik.io/traefik/v3.5/reference/routing-configuration/http/middlewares/ratelimit/) | 이 프로젝트는 Redis 장애와 gateway 보호막을 결합하지 않기 위해 Redis-backed distributed limiter를 기본안으로 쓰지 않는다. Traefik은 instance-local route/global 1차 overload protection이며 authoritative fairness ledger가 아니다. |
...
| Little's Law | Little's Law는 stable system의 평균 체류 수, arrival rate, 평균 체류 시간 사이의 관계를 `L = lambda * W`로 설명한다. | 검증됨 | [Little, A Proof for the Queuing Formula: L = λW](https://pubsonline.informs.org/doi/10.1287/opre.9.3.383) | DB/PG/recovery bulkhead와 gateway limit의 초기 sizing model로 사용하고, 이후 k6/LGTM으로 검증한다. 측정되지 않은 p99를 stable input처럼 쓰면 안 된다. |
| HikariCP pool sizing | HikariCP upstream guidance는 connection pool 과대 설정을 경고하고 realistic load에서 throughput/latency를 측정하라고 강조한다. | 검증됨 | [HikariCP About Pool Sizing](https://github.com/brettwooldridge/HikariCP/wiki/About-Pool-Sizing) | DB write path에는 작고 명시적인 connection/bulkhead budget이 필요하다. connection count 증가는 admission control의 대체물이 아니다. Redis failover 중 새 admission을 DB로 우회하지 않는 근거와도 연결된다. |
...
| Dependency isolation | AWS는 느린 dependency가 concurrency를 증가시켜 thread, memory, socket, connection 같은 제한 자원을 고갈시킬 수 있으므로 bulkhead/concurrency limit로 blast radius를 제한하라고 설명한다. | 검증됨 | [AWS Builders Library: Dependency isolation](https://aws.amazon.com/builders-library/dependency-isolation/) | DB write, PG confirm, recovery worker, Redis failover probe는 같은 WAS 안에 있어도 별도 concurrency/budget을 가져야 한다. |
...
| [Stripe Checkout: Manage limited inventory](https://docs.stripe.com/payments/checkout/managing-limited-inventory?payment-ui=embedded-components) | 공식 결제 문서 | 제한 재고에서 pending sale/session expiration과 재고 반환 흐름을 제공한다. |
| [PayPal API idempotency](https://developer.paypal.com/reference/guidelines/idempotency/) | 공식 결제 문서 | timeout/5xx 이후 같은 idempotency id로 최신 상태를 조회하는 retry 근거를 제공한다. |
| [Cloudflare Waiting Room: About](https://developers.cloudflare.com/waiting-room/about/) | 공식 문서 | FIFO waiting room과 사용자-facing estimated wait update 개념을 제공한다. |
...
3. 프로젝트별 해석은 쟁점 2와 쟁점 6의 decision topic으로 남긴다.
...
| [Traefik v3.5 RateLimit middleware](https://doc.traefik.io/traefik/v3.5/reference/routing-configuration/http/middlewares/ratelimit/) | 공식 문서 | gateway-level overload protection을 위한 token-bucket `average`/`period`/`burst`, source criteria, instance-local/Redis-backed rate limit behavior 근거다. |
...
3. 프로젝트 해석은 user가 수용하기 전까지 쟁점 1/2 design decision으로 남긴다.
...
- 쟁점 7 초기 k6 scenario와 pass/fail metric은 정리되었지만, 구현 후 concrete LGTM dashboard/query와 metric name을 확정해야 한다.
- "user/product당 confirmed booking 1건"이 실제 fairness requirement인지, 아니면 candidate policy인지.
- Traefik instance-local route/global rate limit이 k6 peak test에서 WAS 보호 목적을 충분히 만족하는지.
