...
flowchart LR
    ...
    Checkout --> RedisHA[(Redis HA<br/>cache/admission state)]
    ...
    Booking --> RedisAdmission[Redis HA Admission<br/>Lua + WAIT + pause]
    Booking --> AdmissionLedger[MySQL Admission Ledger<br/>official db_admission_seq]
...
최종 재고 정합성은 Redis가 아니라 MySQL이 보장한다. Redis는 빠른 admission pre-gate일 뿐이다.
...
공정성 기준은 클라이언트 클릭 시각이 아니다. 서버가 검증하고 감사할 수 있는 admission sequence다.

- Redis는 정상 경로에서 provisional sequence와 candidate pool을 빠르게 판단한다.
- MySQL `booking_admission.db_admission_seq`가 공식 순서다.
- 같은 `(sale_event_id, product_id, user_id)`는 admission chance를 하나만 갖는다.
...
현재 채택한 구조는 Redis HA + failover pause다.
...
- 새 admission write 이후에는 `WAIT 1`을 짧게 수행한다.
- failover, WAIT timeout, Redis command timeout이 발생하면 DB로 새 후보를 뽑지 않는다.
...
Redis 장애 중에도 판매를 계속해야 하는 강한 요구가 추가되면, Redis 앞 또는 Redis 옆에 durable admission log를 추가해야 한다.
...
| 쟁점 | 결정 |
|---|---|
| 재고 정합성/공정성 | MySQL admission ledger 공식 순서 + reservation/atomic counter final guard |
| Redis 장애 대응 | Redis HA + WAIT/min-replicas + failover pause + half-open probe |
...
| 과부하 방어 | Traefik 1차 rate limit + app bulkhead + Redis failover controlled rejection |
