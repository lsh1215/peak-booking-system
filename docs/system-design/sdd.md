...
본 시스템은 주문서 진입 정보 조회와 결제/예약 완료 요청을 처리하는 Spring Boot 기반 backend다. k3s + Traefik은 scale-out된 WAS 앞단의 LB/API gateway 역할을 맡으며, `POST /bookings` route-level rate limit으로 WAS 보호를 담당한다. Redis는 정상 상태의 빠른 후보 판단/cache/coordination 컴포넌트로 사용하되, 단일 인스턴스가 아니라 **Redis HA를 기본 장애 대응 구조로** 둔다. Redis failover 중에는 request thread가 MySQL DB fallback으로 직접 우회하지 않고 **WAS-local bounded queue에 요청을 넣은 뒤 throttled worker가 DB 예약 접수 원장을 기록한다**. RDB는 **reservation row + MySQL atomic counter guard로 최종 재고 정합성을** 보장한다.

```mermaid
flowchart LR
    Client[Client] --> LB[Traefik<br/>LB + route-level rate limit]
    LB --> App[Spring Boot App Replicas]
    App --> Redis[(Redis HA<br/>primary + replicas + Sentinel<br/>normal candidate gate/cache)]
    App --> LocalQ[WAS-local bounded queue<br/>Redis outage fallback]
    App --> RDB[(MySQL<br/>예약 접수 원장 + final guard)]
...
- 공정성은 클라이언트 클릭 시각이 아니라 MySQL `booking_admission`에 남은 DB 확정 순서로 판단한다.
- **Redis 장애 시 Booking request thread는 DB 예약 접수 원장으로 직접 우회하지 않는다.** Redis HA failover 중에는 bounded WAS-local queue에 offer하고, worker만 throttled DB 기록을 수행한다.
- **Redis half-open probe가 성공해도 local queue backlog가 남아 있으면 queue drain 또는 복구 시점부터 drain-grace 전까지 새 요청을 Redis 경로로 바로 보내지 않는다.**
...
flowchart TB
    ...
    subgraph App["Spring Boot App Replicas"]
        ...
        RedisAdmission["Redis HA Candidate Gate<br/>Lua + WAIT + failover pause"]
        AdmissionLedger["MySQL 예약 접수 원장<br/>db_admission_seq"]
...
| 사용자/상품 중복 접수 규칙 | 중복 클릭/재시도가 성공 확률을 높이면 안 된다는 방향을 수용. `sale_event_id + product_id + user_id` unique constraint 축으로 방어한다 |
| 예약 접수 원장 | MySQL `booking_admission`을 durable한 공정성/감사 원장으로 두는 방향은 수용. Redis sequence는 provisional 값 |
...
| DDL/Index 원칙 | 단일 surrogate primary key, 비식별 관계, 비즈니스 조건 기반 unique key를 사용한다. 추가 secondary index는 실제 hot query의 equality/range/order 조건과 카디널리티가 명확할 때만 둔다 |
...
- Redis 장애 중 checkout/product metadata cache miss는 booking write-path 후보 판단/failover 정책과 분리한다. checkout read cache fallback은 사용자 표시 정보를 위한 read path이고, booking write-path 후보 판단은 공정성/정합성 write path다.
...
- Redis는 정상 경로의 빠른 후보 판단/cache/coordination 컴포넌트이며, 최종 재고 원장이나 최종 공정성 원장이 아니다.
...
- Redis 후보 write는 Lua script로 처리하고, 새 후보 write에는 짧은 `WAIT 1`을 적용한다.
- HA 모드 Redis server에는 `min-replicas-to-write 1`, `min-replicas-max-lag 1~2s`를 적용한다.
- 새 Redis candidate write 이후 `WAIT`가 실패하면 해당 요청은 MySQL 예약 접수 row를 만들지 않는다. 대신 `ACTIVE_NEW`/`WAITING_NEW`로 반환된 `userId + redisSeq`가 현재 Redis candidate와 일치할 때만 그 신규 candidate를 조건부 보상 삭제한다. 기존 candidate나 Redis 상태 전체는 삭제하지 않는다.
- `WAIT` timeout, min-replica 조건 불만족, Sentinel failover 감지, Redis command timeout이 발생하면 request thread는 새 후보를 DB fallback으로 직접 우회하지 않는다.
...
- local queue worker는 fixed-delay/batch-size budget 안에서만 MySQL 예약 접수 원장을 기록한다.
- local queue worker가 `SERVICE_BUSY`, DB timeout, connection failure를 만나면 해당 entry를 완료 처리하지 않고 retry/backoff 후 재큐잉한다. retry budget 또는 max retry age를 초과한 경우에만 terminal unavailable로 고정한다.
...
- 기존 bounded DB 접수 fallback 후보는 피크에서 DB 보호와 판매 지속성을 동시에 만족시키기 어려워 기본 정책에서 제거한다.
- candidate pool은 sale event당 `30`으로 고정한다.
- 추가 candidate tranche는 열지 않는다. candidate pool 밖 요청은 fast reject한다.

### 7.5 Redis Candidate Gate Design

- Redis는 정상 상태의 fast candidate pre-gate다.
- Redis 자료구조는 ZSET + Hash + String counter를 사용한다.
- Redis 후보 판단의 원자성은 Lua script로 보장한다.
- Redis transaction과 distributed lock은 기본 후보 판단 구현에서 사용하지 않는다.
- Redis sequence만으로는 유효 접수가 아니다. MySQL 예약 접수 row 저장 성공 후에만 유효 접수로 본다.
- Redis HA 모드에서는 새 후보 write 후 `WAIT 1`로 replica ACK를 확인한다. 이는 유실 가능성을 줄이는 완화책이지, durable log와 같은 강한 일관성 보장은 아니다.
- DB write bulkhead는 Redis 후보 판단 전후의 좁은 DB 저장 구간을 보호한다. 이는 DB에 durable한 예약 접수 row를 남길 여력이 없으면 Redis candidate sequence를 먼저 소비하지 않기 위한 선택이다. 따라서 공정성 설명은 "Redis sequence가 최종 기준 순서"가 아니라 "MySQL `booking_admission`에 저장된 `db_admission_seq`가 DB 확정 순서이고 Redis sequence는 빠른 후보 판단/진단값"으로 유지한다.
- Redis 후보 TTL 초기값은 `24h`다. 이는 쟁점 3의 idempotency retention과 맞춘 운영 추적/정리 값이며, correctness나 audit 원장은 MySQL이다.
- Active candidate key는 eviction 대상이 되면 안 되며, Redis persistence는 보조 수단일 뿐 MySQL `booking_admission` table이 복구/감사 원장이다.
- Redis/local queue 세부 결정은 `docs/decisions/DECISIONS.md` 쟁점 2를 따른다.

### 7.6 MySQL 예약 접수 원장

MySQL `booking_admission` table은 공정성/감사 기준이 되는 예약 접수 원장이다. Redis 후보 판단은 이 row가 저장된 뒤에만 유효한 접수로 승격된다.
...
`db_admission_seq`는 `admission_sequence` counter row에서 발급한다. 이 값은 `booking_admission`에 저장된 DB 확정 순서다. 이 counter는 설계상 hot row지만, bounded candidate traffic만 접근하도록 제한한다. 구현 시에는 아래 atomic MySQL update 패턴으로 lock 보유 시간을 줄이는 방향을 우선 고려한다.
...
DB 확정 순서 발급 transaction은 짧게 유지해야 한다. sequence 발급, 예약 접수 row insert, commit까지만 포함하고, payment call, inventory lock, 긴 business processing을 포함하지 않는다.
...
DDL은 단일 surrogate primary key와 비식별 관계를 기본으로 한다. Unique key는 중복 접수, booking attempt, payment effect처럼 실제 비즈니스 불변식이 unique해야 하는 경우에만 둔다.

Secondary index는 조회 조건과 카디널리티를 근거로 둔다. 카디널리티가 낮은 `status` column 단독 index는 기본으로 만들지 않으며, `sale_event_id/product_id`, `booking_attempt_id`, deadline, ordering column처럼 선택도나 접근 패턴이 명확한 column과 묶는다. Query가 느리거나 lock/read pressure가 관측되면 `EXPLAIN`, slow query log, k6/LGTM 결과를 근거로 composite index를 추가한다.

초기 index 설계는 아래 hot path를 기준으로 한다.

| Table | Query / Hot path | Index | 근거 |
|---|---|---|---|
| `sale_inventory` | 재고 atomic update, 재고 조회 | `UNIQUE(sale_event_id, product_id)` | equality 조건으로 단일 counter row를 찾는다. 최종 재고 guard의 핵심 path다. |
| `admission_sequence` | `db_admission_seq` 발급 | `UNIQUE(sale_event_id, product_id)` | event/product별 sequence counter row를 짧게 lock한다. |
| `booking_admission` | 사용자 중복 후보 확인 | `UNIQUE(sale_event_id, product_id, user_id)` | 중복 클릭/재시도가 구매 확률을 높이지 못하게 하는 비즈니스 불변식이다. |
| `booking_admission` | DB 확정 순서 중복 방지, event/product 내 순서 조회 | `UNIQUE(sale_event_id, product_id, db_admission_seq)` | MySQL 예약 접수 원장의 기준 순서를 보존한다. |
| `booking_admission` | status polling, local queue replay의 attempt 조회 | `INDEX(booking_attempt_id)` 또는 non-null uniqueness가 필요하면 `UNIQUE(booking_attempt_id)` | `booking_attempt_id`는 고카디널리티 값이고 status API가 자주 조회한다. nullable이면 MySQL unique key가 여러 `NULL`을 허용한다는 점을 전제로 한다. |
| `booking_admission` | active candidate count, 선순위 waiting 후보 확인 | `INDEX(sale_event_id, product_id, status, db_admission_seq, waiting_expires_at)` | equality column을 앞에 두고, 낮은 카디널리티의 `status`는 event/product 뒤에 둔다. `db_admission_seq` 순서 비교를 함께 탄다. |
| `booking_admission` | 만료 waiting 후보 claim | `INDEX(status, waiting_expires_at, db_admission_seq)` | worker가 `WAITING_CANDIDATE`와 due deadline을 먼저 좁힌 뒤 batch claim한다. `status` 단독 인덱스가 아니라 deadline/order column과 묶는다. |
| `reservation` | attempt 기반 status 조회 | `UNIQUE(booking_attempt_id)` | polling/replay에서 단건 조회한다. |
| `reservation` | stale `HELD` release scan | `INDEX(status, hold_expires_at, id)` | recovery worker가 `HELD` + deadline range + small batch order를 반복 조회한다. |
| `payment_attempt` | attempt 기반 결제 상태 조회/업데이트 | `UNIQUE(booking_attempt_id)` | 대부분의 payment state transition은 attempt 단건 update다. |
| `payment_attempt` | PG provider order idempotency | `UNIQUE(provider_order_id)` | PG confirm side effect 중복을 막는다. |
| `payment_attempt` | due reconciliation claim | `INDEX(status, next_reconcile_at, lease_until, booking_attempt_id)` | recovery worker가 status + due time + lease 조건으로 후보를 좁힌 뒤 reservation과 join한다. |
| `payment_attempt` | reconciliation window 만료 후 manual review 전환 | `INDEX(status, active_reconcile_until)` | `PAYMENT_UNKNOWN`/release 후 reconciliation 상태 중 window가 지난 row만 찾는다. |
| `point_account` | 포인트 잔액 조회/hold | `UNIQUE(user_id)` | user별 account row를 atomic update한다. |
| `point_hold` | attempt별 포인트 hold 조회 | `UNIQUE(booking_attempt_id)` | 현재 모델은 attempt당 point hold가 1개라 `status` 추가 index는 필요 없다. |
| `idempotency_record` | attempt replay / conflict 확인 | `UNIQUE(booking_attempt_id)` | `booking_attempt_id`가 멱등성 기준 key다. |

Recovery query는 `reservation`과 `payment_attempt`를 OR 조건으로 함께 다루기 때문에 MySQL optimizer가 항상 한 개의 composite index만으로 최적 plan을 만들기는 어렵다. `EXPLAIN`에서 full scan, 큰 index merge, filesort가 관측되면 scheduler query를 `HELD deadline`, `REQUESTED/CONFIRMING due`, `PAYMENT_UNKNOWN due`, `release 이후 reconciliation due`처럼 시나리오별로 나누고 위 index를 각각 타도록 조정한다.
...
- `booking_attempt_id`는 Redis에 저장되는 값이 아니다. Redis 후보 상태는 `userId -> redisSeq`, ZSET 순번, TTL을 보관하고, MySQL이 attempt/idempotency/reservation/payment의 권위 있는 저장소다.
- checkout 응답 직후의 `booking_attempt_id`는 signed attempt token이며 아직 DB row가 아닐 수 있다. `POST /bookings` 처리 중 `idempotency_record`가 생성되고, 이후 `booking_admission`, `reservation`, `payment_attempt`, `point_hold`에 단계별로 같은 attempt id가 저장된다.
- Redis 장애 중 `LOCAL_QUEUE_ACCEPTED` 후 local queue worker가 DB 예약 접수 원장에 기록하기 전까지는 attempt가 WAS local memory queue에만 존재할 수 있다. 이 구간은 Kafka/MQ durable queue를 도입하지 않는 대신 감수하는 trade-off다.
...
- `WAITING_CANDIDATE`가 `60s` 안에 선순위 `HELD` 또는 `PAYMENT_UNKNOWN` release를 만나면 `db_admission_seq`의 DB 확정 순서대로 승격한다.
- `60s` 안에 승격되지 않으면 `WAITING_EXPIRED` 또는 sold-out 계열 응답으로 대기를 종료한다.
- **`WAITING_EXPIRED` 이후 같은 `sale_event_id + product_id + user_id`는 새 접수 기회를 받지 않는다.** 같은 사용자의 재요청은 기존 terminal 상태 replay 또는 sold-out 계열 응답이다.
...
| ID | 리스크 | 영향 | 관련 결정 |
|---|---|---|---|
| R-1 | Redis 후보 판단 또는 failover pause 구현이 결정된 candidate/half-open 정책과 다르게 구현될 수 있음 | 높음 | 쟁점 1 / 쟁점 2 |
...
| 공정성 기준 시각 | DB 확정 순서(`db_admission_seq`) | 방향 수용 | 서버 측 순서가 MySQL에 저장되면 측정과 감사가 가능하다. |
...
| Redis coordination | Distributed lock / Redlock | 기본 경로에서 거절 | 후보 판단은 하나의 atomic Lua operation으로 처리할 수 있다. lock은 추가 안전 가정을 만들지만 최종 correctness guard가 되지 않는다. |
| Redis failure fallback | Fail-closed Booking path | 단독 기본안으로 거절 | 가장 단순하지만 Redis 단일 장애 동안 판매가 완전히 멈춘다. |
| Redis failure fallback | Bounded DB 접수 fallback | 기존 후보에서 거절 | Redis down 피크에서 DB 보호와 제한 판매를 동시에 만족시키기 어렵다. |
...
| Redis failure fallback | Durable 접수 로그 추가 | 보류 | Redis 장애 중에도 판매 지속이 강한 요구가 되면 필요하지만, Queue/Kafka/PubSub HA와 consumer 운영 복잡도가 크다. |
| Redis recovery | failover 중 half-open probe 없이 즉시 재개 | 거절 | Sentinel/client 재연결이 안정화되기 전 요청이 긴 timeout으로 몰릴 수 있다. |
| Redis recovery | pause TTL 이후 Redis write + WAIT half-open probe | 수용 | Redis 회복 확인을 제공한다. 단, local queue backlog/drain-grace 정책을 통과해야 새 요청을 REDIS 경로로 보낸다. |
| DB 확정 순서 | `admission_sequence` counter row | 방향 수용 | product/sale event별 DB 확정 순서가 명확하다. 정상 Redis 후보만 접근하므로 hot row 범위를 제한한다. |
| DB 확정 순서 | `AUTO_INCREMENT`를 사용 | 선택하지 않음 | 더 단순하지만 product/sale event별 순서 설명력이 약하고 예약 접수 원장으로 설명하기 어렵다. |
...
- Redis 후보 판단 실패는 통제되지 않은 app exception path가 아니라 Redis failover pause, local queue fallback, half-open recovery로 처리한다.
- Redis failover 중 request thread는 MySQL DB fallback으로 직접 우회하지 않는다. local queue worker만 제한된 속도로 DB 예약 접수 원장을 기록한다.
...
| Redis 후보 판단 | Lua latency, timeout count, duplicate candidate count, BUSY count, candidate pool size, mode transition count |
| DB 예약 접수 원장 | insert latency, `db_admission_seq` issue latency, lock wait, deadlock/timeout count, Hikari active/idle/pending |
...
- **local queue worker가 설정된 batch/fixed-delay budget을 넘어 DB 기록을 폭주시켜도 critical resilience failure다.**
