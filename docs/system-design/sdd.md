...
- 정확한 k3s manifest, Traefik middleware 값, resource request/limit, HPA 사용 여부, readiness/liveness probe.
...
- 최소 `2+` 가정을 넘어 k6 검증에 사용할 정확한 app replica 수.
...
### 15.3 Alert / Pass-Fail 후보

정확한 threshold는 DEC-008에서 결정한다. 초기 alert 후보는 다음과 같다.
...
- fallback 중 Hikari pending connection이 지속 증가하면 DB protection failure로 본다.
- admission sequence 발급 중 DB lock wait timeout 또는 deadlock count가 threshold를 넘으면 sequence hot-row risk로 본다.
- Payment `UNKNOWN` count가 reconciliation window 안에 줄지 않으면 recovery risk로 본다.
...
- p95/p99 latency, error rate, lock wait, Hikari pending, payment unknown drain time에 대한 정확한 SLO/pass-fail threshold.
...
2. Admission status transition과 candidate tranche open 기준은 아직 확정되지 않았다.
