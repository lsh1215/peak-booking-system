...
| 버전 | 0.7 (구현 전 설계 정합성 정리) |
| 상태 | 초안 / 핵심 결정 수용, DDL 작성 전 |
...
- 멱등성은 서버 발급 `booking_attempt_id`, `request_hash`, stored logical response replay, `24h` retention을 사용한다. 이는 요구사항에서 직접 지정한 값이 아니라 DEC-004에서 수용한 설계 결정이다.
...
flowchart TB
    ...
    subgraph App["Spring Boot App Replicas"]
        ...
        RedisAdmission["Redis Admission Gate<br/>ZSET + Hash + String counter + Lua"]
...
- 첫 구현의 candidate pool은 sale event당 `30`으로 고정한다.
- 추가 candidate tranche는 열지 않는다. candidate pool 밖 요청은 fast reject한다.
- rate limit, semaphore/connection budget은 DEC-007의 초기 runtime budget을 적용하고 k6/LGTM 결과로 조정한다.
...
- Redis admission TTL 초기값은 `24h`다. 이는 DEC-004 idempotency retention과 맞춘 운영 추적/정리 값이며, correctness나 audit 원장은 MySQL이다.
...
| ID | 리스크 | 영향 | 관련 결정 |
|---|---|---|---|
| R-1 | Redis admission 또는 DB fallback 구현이 결정된 `sale_event_id`/candidate 정책과 다르게 구현될 수 있음 | 높음 | DEC-001 / DEC-002 |
| R-2 | bound 설정이 실제 환경과 맞지 않으면 Redis fallback이 RDB를 과부하시킬 수 있음 | 높음 | DEC-002 / DEC-007 |
| R-3 | Inventory guard 구현이 counter와 reservation 상태 전이를 같은 transaction으로 묶지 못할 수 있음 | 치명적 | DEC-003 |
| R-4 | 빠른 반복 결제 요청이 `request_hash`/replay 정책을 우회할 수 있음 | 치명적 | DEC-004 |
| R-5 | recovery worker의 lease 구현 오류가 duplicate status/cancel 또는 stale update를 만들 수 있음 | 높음 | DEC-005 |
| R-6 | k6/LGTM metric 구현이 pass/fail 기준과 어긋나 검증 근거가 약해질 수 있음 | 중간 | DEC-008 |
...
| NFR-3 | 실행 가능한 소스와 문서 | §17 | DEC-008 |
...
| Redis recovery | 같은 `sale_event_id`에서 Redis로 복귀 | 거절 | Redis ordering과 DB fallback ordering을 병합하면 공정성이 깨질 수 있다. |
| Redis recovery | 같은 `sale_event_id`에서 sticky `DB_FALLBACK` 유지 | 수용 | 더 단순하고 ordering 병합 문제를 피한다. |
| DB admission sequence | `admission_sequence` counter row | 방향 수용 | product/sale event별 공식 sequence가 명확하다. hot row는 bounded 처리와 테스트가 필요하다. |
...
구현은 TDD로 진행한다. 작은 단위의 실패 테스트를 먼저 작성하고, 구현이 커질수록 slice/integration/acceptance test로 범위를 넓힌다. k6 부하 테스트는 구현 완료 뒤 staging과 유사한 환경에서 실행해 DEC-007/DEC-008의 runtime budget과 pass/fail 기준을 검증한다. k6 검증 범위에는 정상 부하뿐 아니라 부하 중 WAS/Redis/PG 일부 장애를 주입하는 resilience load test가 포함된다.
...
| k6 Resilience / Failure Mix | Redis down, WAS 1대 down, PG timeout/unknown, duplicate storm, spike 혼합 | 일부 구성요소 장애 중에도 hard correctness fail 없이 통제된 응답과 복구 상태 전이를 유지하는가 |

부하 테스트는 unit/integration TDD를 대체하지 않는다. unit/integration test는 구현 전에 작성하고, k6는 구현 후 local 또는 staging-like 환경에서 실행한다. 정상 부하만 통과하는 것은 충분하지 않으며, Redis 장애 시 bounded DB fallback, WAS 1대 장애 시 LB/생존 replica 처리, PG timeout/unknown 시 `PAYMENT_UNKNOWN`/recovery 전이, 중복 클릭 폭주 시 멱등성 replay가 부하 상태에서 함께 검증되어야 한다. 장시간 soak/endurance test와 destructive stress test는 첫 구현의 필수 pass/fail 기준이 아니다.
...
| Fallback | `NORMAL_REDIS` vs `DB_FALLBACK` mode, fallback admission accepted/rejected count, candidate pool accepted/rejected count |
...
- Hikari pending이 `30s` 이상 증가하면 DB write/fallback bulkhead를 줄이고 query/lock을 분석한다.
